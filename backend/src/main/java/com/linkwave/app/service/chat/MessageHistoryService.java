...
public class MessageHistoryService {
        ...
        public MessageHistoryService(ChatMessageRepository messageRepository,
        ...
        public List<MessageDTO> getDirectMessageHistory(String currentUserPhone, String otherUserPhone) {
                ChatRoomEntity room = findDirectRoomBetweenUsers(currentUserPhone, otherUserPhone)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "No conversation found with this user"));

                Page<ChatMessageEntity> messages = messageRepository.findByRoomOrderBySentAtDesc(
                                room,
                                PageRequest.of(0, MAX_MESSAGES_PER_REQUEST));
        ...
        public PaginatedMessagesResponse getDirectMessageHistoryPaginated(
                ...
                if (before != null) {
                        messagePage = messageRepository.findByRoomAndSentAtBeforeOrderBySentAtDesc(
                                        room, before, pageRequest);
                } else {
