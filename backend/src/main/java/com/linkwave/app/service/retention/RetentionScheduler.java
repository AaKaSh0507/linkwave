...
public class RetentionScheduler {
  ...
  public RetentionScheduler(
      RetentionService retentionService, RetentionProperties retentionProperties) {
  ...
  @SchedulerLock(name = "messageRetentionCleanup", lockAtMostFor = "PT2H", lockAtLeastFor = "PT5M")
  public void executeRetentionCleanup() {
    log.info(
        "Starting scheduled message retention cleanup. retentionDays={}",
    ...
    try {
      ...
      if (result.successful()) {
        log.info(
            "Scheduled retention cleanup completed successfully. totalMessagesDeleted={},"
                + " batchesProcessed={}, executionTimeMs={}",
            result.totalMessagesDeleted(),
            result.batchesProcessed(),
            result.executionTimeMs());
      } else {
        log.error(
            "Scheduled retention cleanup completed with errors. totalMessagesDeleted={},"
                + " batchesProcessed={}, executionTimeMs={}, error={}",
            result.totalMessagesDeleted(),
            result.batchesProcessed(),
            result.executionTimeMs(),
            result.errorMessage());
