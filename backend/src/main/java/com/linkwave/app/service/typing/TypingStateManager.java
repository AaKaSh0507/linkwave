...
public class TypingStateManager {
  ...
  public void markTypingStop(String roomId, String userId, String sessionId) {
    ...
    if (typingUsers != null) {
      typingUsers.removeIf(
          state -> state.userId.equals(userId) && state.sessionId.equals(sessionId));
  ...
  public Set<String> getTypingUsers(String roomId) {
    ...
    return typingUsers.stream().map(state -> state.userId).collect(Collectors.toSet());
  }
  ...
  public List<String> clearUserTyping(String userId, String sessionId) {
    ...
    if (!affectedRooms.isEmpty()) {
      log.debug(
          "Cleared typing state for user {} in {} rooms", maskUserId(userId), affectedRooms.size());
    }
  ...
  public TypingStats getStats() {
    ...
    int totalTypingUsers = roomTypingState.values().stream().mapToInt(Set::size).sum();
