...
public class RetentionService {
  ...
  public RetentionService(
      RetentionBatchDeleter retentionBatchDeleter, RetentionProperties retentionProperties) {
  ...
  public RetentionResult deleteExpiredMessages() {
    ...
    log.info(
        "Starting message retention cleanup. cutoffTimestamp={}, retentionDays={}, batchSize={}",
        cutoffTimestamp,
    ...
    try {
      ...
      if (totalDeleted > 10000) {
        log.info(
            "Large deletion detected. Consider running VACUUM ANALYZE on chat_messages table for"
                + " optimal performance.");
      }
