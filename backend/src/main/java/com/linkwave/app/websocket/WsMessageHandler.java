...
import com.linkwave.app.service.kafka.ChatMessageProducer;
...
public class WsMessageHandler extends TextWebSocketHandler {
    ...
    private void handleChatSend(WebSocketSession session, String phoneNumber, WsMessageEnvelope envelope)
        ...
        // Create chat message
        ChatMessage chatMessage = ChatMessage.create(phoneNumber, envelope.getTo(), body);

        // Publish to Kafka
        chatMessageProducer.publishChatMessage(chatMessage)
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        // Send acknowledgment to sender
                        try {
                            sendChatSentAck(session, chatMessage.getMessageId());
                        } catch (Exception e) {
                            log.error("Failed to send chat.sent acknowledgment: {}", e.getMessage());
                        }
                    } else {
                        log.error("Failed to publish chat message to Kafka: {}", ex.getMessage());
